...
class Post_Model extends MY_Model {
	...
	var $hot_conf;
	var $ugc_need_check; //2013.12.27 add zr
	...
	public function get_user_timeline($id){
		...
		{
			$this->db->where(array($col_id=>$id, 'status'=>1 )); 
		}
		else{
...
// 		}
		if($this->ugc_need_check==1){
			$list = $this->db->where(array('status'=>1, 'type < '=>5, 'placeId'=>$place_id))->limit($size, $offset)->get($this->_tables['post'])->result_array();
		}
		else{
...
//  			$row['is_praised'] = $is_praised;
	...
	public function get_post($post_id){
		...
		if($this->ugc_need_check==1){
			$info = $this->db->where(array($this->_tables['post'].'.id'=>$post_id,$this->_tables['post'].'.status'=>1))->limit(1)->get($this->_tables['post'])->first_row('array');
		}else{
		...
		if(!empty($info)){
			...
			$info['prev_id'] = get_data_ttl('post_prev',$post_id.'-'.$info['type'].'-'.intval($info['placeId']),7200);
			//下一条，-1=没有下一条  
			...
			$info['next_id'] = get_data_ttl('post_next',$post_id.'-'.$info['type'].'-'.intval($info['placeId']),7200);
			//POST标题
			$str = '';
			$vico = '';
	...
	public function list_post_for_tag($post_id, $size=0, $offset=0, $pic_size='odp'){
		...
		if($tags){//关联了TAG的
			...
			*/
			...
			$uid && $this->db->join($this->_tables['usermnemonic'], $this->_tables['usermnemonic'].'.mUid='.$this->_tables['user'].'.id AND '.$this->_tables['usermnemonic'].'.uid='.$uid, 'left');
			if($this->ugc_need_check==1){
				$this->db->where(array($this->_tables['post'].'.placeId'=>$p['placeId'],$this->_tables['post'].'.type < '=>5, $this->_tables['post'].'.status'=>1, $this->_tables['post'].'.id != '=>$post_id, $this->_tables['post'].'.replyCount > '=>0));			
			}else{
