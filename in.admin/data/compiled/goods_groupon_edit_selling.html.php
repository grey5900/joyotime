<?php if (!defined('BASEPATH')) exit('No direct script access allowed');?><div class="pageContent" id="groupon_edit_selling">
    <form method="post" action="<?=$current_url?>" class="pageForm required-validate" onsubmit="return validateCallback(this, navTabAjaxDone);">
        <div class="pageFormContent nowrap" layoutH="57">
            <dl>
                <dt>商品标题：</dt>
                <dd><input type="text" id="title" name="title" class="required" size="80" maxlength="80" value="<?=$groupon['title']?>" /></dd>
            </dl>
            <dl>
                <dt>商品描述：</dt>
            ...
            <dl>
                <dt>缩略图：</dt>
                <dd><button id="groupon_edit_selling_image" type="button">上传缩略图</button></dd>
            </dl>
            <dl>
                <dt>使用提示：</dt>
            ...
            <dl>
                <dt>节省：</dt>
                <dd><input type="text" id="saving" name="saving" class="required number" size="10" readonly="true" maxlength="20" value="<?php echo $groupon['marketPrice'] - $groupon['grouponPrice']?>" /> 元</dd>
            </dl>
            <dl>
                <dt>折扣：</dt>
                <dd><input type="text" id="discount" name="discount" class="required number" size="10" readonly="true" maxlength="20" value="<?php echo intval($groupon['marketPrice'])?number_format($groupon['grouponPrice']*10/$groupon['marketPrice'], 1):0?>" /> 折</dd>
            </dl>
            <dl>
                <dt>开始时间：</dt>
...
<script type="text/javascript">
    $(function(){
        var options = "name:\"groupon_edit_selling_image\", id:\"groupon_edit_selling_image\", file_type:\"common\"";
        <?php if($groupon['imageUri']) { ?>
            options += ", image_name: \"<?=$groupon['imageUri']?>\"";
        <?php } ?>
        $("#groupon_edit_selling_image").my_upload(eval("({" + options + "})"));
        
        // 地点的选择
        $("#search", $("#groupon_edit_selling")).click(function(){
            var keywords = $.trim($("#keywords", $("#groupon_edit_selling")).val());
            if("" == keywords) {
                alert("请输入查询的关键词");
                return;
            }
            $("#source_place", $("#groupon_edit_selling")).html("");
            $.getJSON("<?php echo site_url(array('goods', 'place', 'keywords'))?>/" + encodeURIComponent(keywords), function(data){
                var len = data.length;
                for(var i=0; i<len; i++) {
                    var option_str = "<option value=\"" + data[i].id + "\">" + "(" + data[i].id + ")" + data[i].placename + "</option>";
                    $("#source_place", $("#groupon_edit_selling")).append(option_str);    
                }
            });
        });
        
        $("#add_btn", $("#groupon_edit_selling")).click(function(){
            // 获取所有备选
            move_selling_list("source_place", "place");
        });
        
        $("#source_place", $("#groupon_edit_selling")).dblclick(function(){
            // 双击
            move_selling_list("source_place", "place");
        });
        
        $("#del_btn", $("#groupon_edit_selling")).click(function(){
            // 删除已经选中的
            move_selling_list("place", "source_place");
        });
        
        $("#place", $("#groupon_edit_selling")).dblclick(function(){
            // 双击
            move_selling_list("place", "source_place");
        });
    
        // 初始之前的选中的
        <?php if(is_array($place)) { foreach($place as $p) { ?>
        $("#place", $("#groupon_edit_selling")).append("<option value=\"<?=$p['id']?>\">(<?=$p['id']?>)<?=$p['placename']?></option>");
        <?php } } ?>
...
function move_selling_list(id1, id2) {
    var selected_items = $("#" + id1, $("#groupon_edit_selling")).find("option:selected");
    ...
    for(var i=0; i<selected_items.length; i++) {
        ...
        $("#" + id2, $("#groupon_edit_selling")).append(option_str);
    }
}
...
function onsubmitSelectedSelling(){
    $("#place", $("#groupon_edit_selling")).find("option").attr("selected", "selected");
}
</script>
