 ...
 * @param POST的ID号 $id
 */
function cache_func_post_prev($idNtypeNplaceid) {
    static $db = null;
    ...
    $id = explode('-',$idNtypeNplaceid)[0];
    $type = explode('-',$idNtypeNplaceid)[1];
    $placeid = intval(explode('-',$idNtypeNplaceid)[2]);
    
    $prev = $db->select('id')->where(array('id < '=>$id, 'type'=>$type, 'status < '=>2, 'placeId'=>$placeid))->order_by('id', 'desc')->limit(1)->get('Post')->first_row('array');
	$prev_id = empty($prev) ? -1 : $prev['id'];
 ...
 * @param POST的ID号 $id
 */
function cache_func_post_next($idNtypeNplaceid) {
    static $db = null;
    ...
    $next = $db->select('id')->where(array('id > '=>$id, 'type'=>$type, 'status < '=>2, 'placeId'=>$placeid))->order_by('id', 'asc')->limit(1)->get('Post')->first_row('array');
	$next_id = empty($next) ? -1 : $next['id'];
