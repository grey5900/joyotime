...
//             }
     ...
     * 批量处理报错 2013 12 10
     * */
    function batch_deal($give_credit = 0){
        /*
         * 报错修改成已处理状态
		 * 给用户积分 私信
		 * 修改地点确认状态为已确认
    	...
    	// 发送消息
        $this->load->helper('ugc');
    	$placeids = $this->post('ids');  	
    	
    	
    	
    	$places = $this->db->select('id,status,isConfirm,placename')->where_in('id',$placeids)->get('Place')->result_array();	
    	$poi_status = $poi_name = array();
    	foreach($places as $row){
    		$poi_status [$row['id']] = $row['status'];
    		//$poi_confirm [$row['id']] = $row['isConfirm'];
    		$poi_name [$row['id']] = $row['placename'];
    	}
    	
    	
    	
    	$uids_list = $this->db->select('id,placeId,uid')->where_in('placeId',$placeids)->where('status',0)->get('PlaceErrorReport')->result_array();
    	
    	$uids = $places = array();
    	
    	foreach($uids_list as $row){
    		if(!in_array($row['uid'],(array)$uids[$row['placeId']])){
	    		$uids [$row['placeId']] [] = $row['uid'];
	    		$places [$row['placeId']] [] = array('place'=>$poi_name[$row['placeId']]);
    		}
    	}
    	
    	//报错修改成已处理状态
    	$update_status = $this->db->where_in('placeId',$placeids)->where('status',0)->update('PlaceErrorReport',array('status'=>1));
    	//地点isConfirm
    	$update_place_status = $this->db->where_in('id',$placeids)->update('Place',array('isConfirm'=>1));
    	
    	if($update_status && $update_place_status){   	
    		
    		foreach($uids as $k=>$uid){			
    			($give_credit && $uid) && make_point($uid, 'poi_report', "0", $k);
    			if($give_credit && $uid) {				
    				send_message('sm_poi_report_confirm', $uid, $poi_status[$k]==2?0:$k, $poi_status[$k]==2?0:1, true, $places[$k]);
    			}
    		}
    		// 更新地点缓存
	        api_update_cache('Place', $placeids);
	        // 更新分类关系
	        api_update_cache('PlaceCategoryShip');  
	        foreach($placeids as $pid){
	         	send_api_interface('/private_api/place/update_solr', 'POST', array('place_id' => $pid));
	        }        
	        $this->success('批量处理报错POI成功', $this->_index_rel, $this->_index_uri, 'forward');
    	}  	
    	$this->error('批量处理报错POI失败', $this->_index_rel, $this->_index_uri, 'forward');   
    }
}
