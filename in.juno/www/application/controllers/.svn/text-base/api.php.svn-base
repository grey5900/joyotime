...
class Api extends Controller {
     ...
     * 
     * 调用地点册接口
     * $opration list/detail
     * $ids array()
     */
    function placecoll($opration = 'list',$ids = '' , $page = 1 , $pagesize = 100 , $order_by = 'new',$start_date = '',$callback = '?', $force = 1800, $is_count = 0,$keyword = ''){
        ...
        $cache_id = 'cache_placecoll_' . implode('_', $params);
        $data = xcache_get($cache_id);

        if (empty($data) || $force) {
	        $this->load->model("placecollection_model","m_placecoll");
	        if($opration == "list"){
	        	$where = '';
		        if($ids){
		        	$ids = explode('-',$ids);
		        ...
		        if($start_date){
		        	$where .= " and createDate > '{$start_date}' ";
		        }
		        if($keyword){
		        	$keyword = urldecode($keyword);
		        	$where .= " and ( name like '%{$keyword}%' or summary like '%{$keyword}%' ) ";
		        }
	        	$list = $this->m_placecoll->list_place_collection($page,$pagesize,$order_by,$where);
	        	
	        	foreach($list as &$row){
	        		$row['image'] = image_url($row['image'],'placeColl','pcdp');	   
	        	}
	        	
	        	$data['data'] = $list;
	        	if($is_count){
	        		$count = $this->m_placecoll->count_place_collections($order_by,' 1=1 '.$where);
	        		$data['count'] = $count;
	        	}
	        }
	        else{
	        	if($ids){
			        $list = $this->m_placecoll->get_placecollection(intval($ids),$page,$pagesize);
			        $list['image'] = image_url($list['image'],'placeColl','pcdp');
			        
			        unset($list['ps_list'],$list['user']);
					$data = $list;
	        	}else{
	        		$data = array();
	        	}
	        }
	        xcache_set($cache_id, $data, $force);
    ...
    function ciis($index = '') {
        ...
        $size = explode('x', trim($this->get('size')));
        $w = intval($size[0]);
        $h = intval($size[1]);
        $w = $w>50?$w:0;
        $h = $h>50?$h:0;

        $this->assign(array('w' => $w, 'h' => $h));
        
        $this->display('ciis');
    }
    
	function ciis2($index = '') {
        $full = $this->get('full');
        $this->assign('full', $full?$full:40);
     ...
     * @param $force 是否强制更新 默认 0  <0强制更新 >0缓存时间
     * @param $is_count 是否需要统计 默认1 需要统计 1/0
     */
    function feed($page = 1, $page_size = 100, $place_id = 0, $type = 0, $content = '', $id = 0, $post_date = '', $order = 1, $callback = '?', $force = 1800, $is_count = 0, $topic_id =0, $have_photo = 0,$order_by = 'p.createDate') {
        ...
        if (empty($data) || $force) {
            ...
            //$have_photo = $this->_format_params('p.photo <>', '');
            //$have_photo && $where_sql[] = '';
            ...
            $have_photo && $where_sql .= " and p.photo <> '' ";
            ...
            if($is_count)
            {
            				$this->db->from('Post p');
						            if($topic_id){
						            	$topic_array = explode('-',$topic_id);
						            	$this->db->join('TopicOwnPost top', 'top.postId = p.id','left');
						            	$this->db->where_in('top.topicId',$topic_array);
						            }
             $total24 = $this->db->where($where_sql . (" and p.createDate between '{$d1}' and '{$d2}'"),null,false)->count_all_results();
            }
            ...
            // 获取总的数据
            	if($is_count){
						          ...
						          $total = $this->db->where($where_sql,null,false)->count_all_results();
            	}

            $order = intval($order);
            $order = $order ? 'desc' : 'asc';

            if (empty($is_count) || $total) {
                $this->db->select('p.*')->from('Post p');
            						if($topic_id){
						            	$topic_array = explode('-',$topic_id);
						            	$this->db->join('TopicOwnPost top', 'top.postId = p.id','left');
						            	$this->db->where_in('top.topicId',$topic_array);
						            }
                $list = $this->db->where($where_sql,null,false)->order_by($order_by, $order)->limit($page_size, ($page - 1) * $page_size)->get()->result_array();
                ...
                if ($list) {
                    ...
                    foreach ($list as $row) {
                        ...
                        $r['post_url'] = base_url() . 'review/' . $row['id'];
                        ...
                        if ($row['photoName']) {
                            ...
                            $image = get_data('imagesize', $row['photoName']);
                            if ($image !== false) {
                                $r['photo_width'] = $image['w'];
                                $r['photo_height'] = $image['h'];
                            }
                        }
                        $r['praiseCount'] = $row['praiseCount'];
                        $r['content'] = $row['content'];
                        $r['gender'] = $u[$row['uid']]['gender'];
						$r['photo'] = $row['photo']?image_url($row['photo'], 'user', 'thweb'):'';
						$r['original'] = $row['photo']?image_url($row['photo'], 'user', 'hdp'):'';
                        $data['list'][] = $r;
                    }
                    unset($list);
                }
     ...
     * @param $is_count 是否需要统计 默认1 需要统计 1/0
     */
	function users($page = 1, $page_size = 100, $keywords = '', $id = 0, $order = 1, $callback = '?', $force = 1800, $is_count = 0) {
        ...
        $cache_id = 'user_cache_' . implode('_', $params);

        $data = xcache_get($cache_id);

        if (empty($data) || $force) {
            // 状态为0的
            $where_sql = array('u.status <= 0');
            ...
            // 对应的信息ID号post id
            $id = $this->_format_params('u.id', $id);
            $id && $where_sql[] = $id;

            // 匹配内容
            $keywords = trim(urldecode($keywords));
            if ($keywords && substr_count($keywords, '%') != count($keywords)) {
                // 有搜索内容且搜索内容不是%
                $keywords = addslashes($keywords);
                $where_sql[] = "u.username like '%{$keywords}%' or u.nickname like '%{$keywords}%' ";
            }

            $where_sql = implode(' and ', $where_sql);
                        

            $is_count && $total = $this->db->from('User u')->where($where_sql)->count_all_results();

            $order = intval($order);
            $order = $order ? 'desc' : 'asc';

            if (empty($is_count) || $total) {
                $list = $this->db->select('u.id,u.avatar,u.nickname,u.username ')->from('User u')->where($where_sql)->order_by('u.createDate', $order)->limit($page_size, ($page - 1) * $page_size)->get()->result_array();
                ...
                if ($list) {                 
                    foreach ($list as $row) {
                        ...
                        $r['username'] = $row['nickname'] ?  $row['nickname']  :  $row['username'] ;
                        $r['avatar'] = image_url($row['avatar'],'head');
    ...
    function anniversary(){
    	...
    	$this->auth['uid'] = $uid;
    	$res = request_api('/props/anniversary','POST',array(),array ('uid' => $this->auth ['uid'] ),true);
    	//$res = call_api('/props/anniversary',array('uid'=> $this->auth['uid']),'POST',true);
    	echo $res ;exit;
    }
    ...
    function zztl(){
    	...
    	if($r['result_code']=="13045"){
    		xcache_set("zztl_prize_runout",1,3600*24*30);
    	}
    	echo $res ;exit;
    }
    
    function reco_list_for_news($callback = ''){
    	$_callback = $this->input->get('callback');
    	$_callback && $callback = $_callback;
    	//从碎片管理中的 “IN成都推荐到新闻内页的侧边栏” 推荐的内容读取出来 
    	$fragmentId = 238 ;
    	$list = $this->db->where('fragmentId',$fragmentId)->order_by('orderValue','asc')->limit(6)->get('WebRecommendData')->result_array();
    	$data = array();
    	foreach($list as $k=>$row){
    		$data[$k]['title'] = $row['title'];
    		$data[$k]['link'] = $row['link'];
    	}
    	echo $callback ? $callback.'('.json_encode($data).')' : json_encode($data);
    }
