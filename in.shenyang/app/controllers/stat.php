...
//                         
    ...
    function user_stat_general() {
        ...
        foreach($list_yesterday as $key => $row) {
            ...
            $row['activeCount'] = tmp_boost_stat($yesterday, $row['activeCount'], 1) + ($key?1:0);
            $row['connectCount'] = tmp_boost_stat($yesterday, $row['connectCount'], 0) + ($key?1:0);
    ...
    function user_stat_detail() {
        ...
        if ($this->is_post()) {
            ...
            $list = $this->db->select('statDate')->select_sum('newCount')
                             ...
                             ->limit($paginate['per_page_num'], $paginate['offset'])
                             ->order_by('statDate', 'desc')->get('StatUserDevice')->result_array();
            if(empty($end_date)) {
                // 去获取开始时间
            ...
            foreach($list as &$row) {
                $row['connectCount'] = tmp_boost_stat($row['statDate'], $row['connectCount'], 0);
                $row['activeCount'] = tmp_boost_stat($row['statDate'], $row['activeCount'], 1);
                $row['activeCount7'] = tmp_boost_stat($row['statDate'], $row['activeCount7'], 2);
                $row['activeCount15'] = tmp_boost_stat($row['statDate'], $row['activeCount15'], 3);
            }
            unset($row);
    ...
    function user_stat_analysis() {
        if ($this->is_post()) {
            ...
            for($i=0; $i<$len; $i++) {
                ...
                $row['connectCount'] = tmp_boost_stat($row['statDate'], $row['connectCount'], 0);
                $row['activeCount7'] = tmp_boost_stat($row['statDate'], $row['connectCount'], 2);
...
function tmp_boost_stat($datetime, $num, $type = 0) {
    return $num;
}
