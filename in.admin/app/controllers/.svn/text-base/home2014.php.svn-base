...
class Home2014 extends MY_Controller {
	...
	function hots($do = 'post') {
		...
		$home_digest_conf = $this->config->item('home_digest');
		$where_sql = "type = '" . $home_digest_conf['home_' . $do]['id'] . "'";
		if($keywords !== '') {
...
// 			}
			$item_type = 0;
			$item_id = 0;
			$home_link_type_conf = $this->config->item('home_link_type');
			$schema = $home_link_type_conf[$link_type]['schema'];
			if (!in_array($link_type, array('http', 'any'))) {
				// 那么就都是填写的id号
				$item_type =$home_link_type_conf[$link_type]['type'];
...
// 			}
			...
			if(empty($data)) {
				// 添加
				$home_digest_conf = $this->config->item('home_digest');
				$data1['type'] = $home_digest_conf['home_' . $do]['id'];
			}
		...
		$this->assign('label_title', $home_digest_conf['home_' . $do]['label']);
	...
	function del($do = 'post', $id = 0) {
		...
		if($id > 0) {
			$home_digest_conf = $this->config->item('home_digest');
			$this->db->where(array('id' => $id, 'type' => $home_digest_conf['home_' . $do]['id']))->delete('HomePageDataBase');
		}
