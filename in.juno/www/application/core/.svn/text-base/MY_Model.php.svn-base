...
class MY_Model extends CI_Model {
	...
	function __call($method, $args) {
		if(method_exists($this, $method)) {
			...
			// 得到表名
			$table = $this->_tables[explode('_', $class_name)[0]];
			// 获取操作
			...
			if($need_where) {
				...
				switch($method_part[1]) {
					...
					default:
					    // 情况，直接把参数当做条件
					    isset($args[0]) && $this->db->where($args[0]);
				}
			}
			...
			switch($method_part[0]) {
				...
				case 'list':
					...
					isset($args[1])?(isset($args[2])?$this->db->limit($args[1], $args[2]):$this->db->limit($args[1])):'';
