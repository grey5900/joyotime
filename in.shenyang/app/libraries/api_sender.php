...
class Api_sender{
	...
	function post_api($params){
		...
		$query_str = '';
		if(!empty($params['attr']) && is_array($params['attr'])){
			if(empty($params['attr']['uploaded_file'])){
				$query_str = http_build_query($params['attr']);
				$sign = $this->openssl_encrypt();
				$query_str .= '&sign='.urlencode($sign);
			}else{//上传图片
				$query_str = $params['attr'];
				$query_str['sign'] = urlencode($sign = $this->openssl_encrypt());
			}
		}
		//构造curl
		$curl = curl_init();
