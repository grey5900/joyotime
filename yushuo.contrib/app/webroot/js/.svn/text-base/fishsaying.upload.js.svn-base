!function ($) {
  ...
  Fileupload.prototype = {
      ...
      if (this.type === "file") {
        ...
        // var $m4a = typeof file.type !== "undefined" ? file.type.match('m4a') : file.name.match(/\.(m4a)$/i); 
        // console.log($m4a);
        // if($m4a) {
            ...
            reader.onload = function(e) {
                var audio = new Audio();
                var voiceURL = e.target.result;
                    
                    $("[data-id='voices_voice']").attr('src',voiceURL);
                    //audio = $('[data-id="voices_voice"]')[0];
                    
                var audio = document.getElementById('voices_voice');
                    audio.src = voiceURL;
                var seconds = audio.duration;
                    ...
                    audio.addEventListener('loadedmetadata', function() {
                        ...
                        if(audio.duration < 60) {
                            $.messager('音频必须在5分钟内，60秒之上。');
                            $('#voiceDuration').val('');
                            $this.$input.val('');
                            $this.$preview.html('');
                            $this.$element.addClass('fileupload-new').removeClass('fileupload-exists');
                        } else if(audio.duration > 301)  {
                            $.messager('音频必须在5分钟内，60秒之上。');
                            $('#voiceDuration').val('');
                            $this.$input.val('');
                            $this.$preview.html('');
                           ...
                           $('#voiceDuration').attr('value',audio.duration); 
                        }
                    },false);
  ...
  function previewCrop(img, selection) {
    ...
    var pit = boxW + boxH;
    var scales = PIT / pit
    $('#l').val(Math.round(scales * selection.x1));
    $('#t').val(Math.round(scales * selection.y1));
    $('#w').val(Math.round(scales * selection.width));
    $('#h').val(Math.round(scales * selection.height));
  }
