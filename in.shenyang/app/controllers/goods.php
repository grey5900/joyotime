...
class Goods extends MY_Controller {
    ...
    function groupon_source() {
        ...
        $type && $where_sql .= " AND sourceType = '" . $source_type[$type] . "'";
        if($keywords !== '') {
...
// 
    ...
    function groupon_selling() {
        ...
        switch($status) {
            case 1:
                // 售卖中
                $where_sql = "gi.status = 0 and '{$datetime}' between gi.startDate and gi.endDate";
                break;
            case 2:
                // 未开始
                $where_sql = "gi.status = 0 and gi.startDate > '{$datetime}'";
                break;
            case 3:
                // 已结束
                $where_sql = 'gi.status = 2';
                break;
            default:
                // 全部
                $where_sql = 'gi.status in (0, 2)';
                $status = 0;
        }
