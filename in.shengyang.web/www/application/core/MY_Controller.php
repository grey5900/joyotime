...
// 		list($key, $value) = pre_google2baidu($key, $value);
...
function pre_google2baidu($key, $value = null) {
	if(is_array($key)) {
		foreach($key as $k => $v) {
			if(is_array($v) && $k != 'baidu_map') {
				list($v, ) = pre_google2baidu($v);
				$key[$k] = $v;
			} else {
				if($k == 'baidu_map') {
					$key['baidu_map'] = google2baidu($key['baidu_map']['lat'], $key['baidu_map']['lng']);

					$key['lat'] = $key['baidu_map']['lat'];
					$key['latitude'] = $key['baidu_map']['lat'];
						
					$key['lng'] = $key['baidu_map']['lng'];
					$key['longitude'] = $key['baidu_map']['lng'];
				}
			}
		}
	} else {
