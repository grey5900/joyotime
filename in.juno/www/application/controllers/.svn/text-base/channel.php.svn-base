 ...
 */
define(CHANNEL_REDIRECT_CODE, 301); //频道使用301跳转，seo友好
class Channel extends Controller {
    ...
    function __construct() {
        ...
        if('index' == $segment[2]) {
            ...
            if(empty($this->domain)) {
                ...
                if(HOST == $in_host) {
                    ...
                    redirect(HTTP_SCHEMA_STR . $in_host, '', CHANNEL_REDIRECT_CODE); // 最后要改成301
                }
            } else {
            ...
            if($this->uri->segments && !in_array($this->uri->segments[1], array('new', 'hot'))) {
                redirect(HTTP_SCHEMA_STR . ($this->id?HOST:$in_host), '', CHANNEL_REDIRECT_CODE);
            }
            ...
            if($this->id > 0) {
                ...
                if(empty($host)) {
                    ...
                    (HOST != $in_host) && redirect(HTTP_SCHEMA_STR . $in_host . '/' . uri_string(), '', CHANNEL_REDIRECT_CODE);
                	}
                } else {
                    // 如果不是在本来的频道下
                    if($segment[2]!='all_posts'){
                    ($host != HOST) && redirect(HTTP_SCHEMA_STR . $host . '/' . uri_string(), '', CHANNEL_REDIRECT_CODE);
                    }
                }
        ...
        if($this->domain) {
        	$channel_domain = $this->big_category['domain'] ? $this->big_category['domain'] : $this->category['domain'];
            // 获取频道下的栏目
            $this->assign(array(
                    ...
                    'in_host' => $channel_domain ? $channel_domain : $in_host,
    ...
    private function _channel($id, $tagid = 0 ,$sort = 'new',$subpage=1,$curr_page = 1) {
    	...
    	$id = formatid($id);
    	$curr_page = formatid($curr_page , 1);
    	$tagid = formatid($tagid);
    	$subpage = formatid($subpage , 1);
        ...
        //成都美食_成都美食网   //评房网_成都房地产新闻
     	if($this->title=="美食" ){
        	$this->title = '成都美食_成都美食网';     	
        }
    ...
    function article($id, $page = 1 ) {
    	...
    	//echo $this->big_category['id'], $this->category['id'];
    	
    	$fang_id = 3; //评房网id
    	
    	$fragments = $this->category['fragmentId'];
    	
    	if($this->big_category['id'] == $fang_id){
    		$show_fang_weixin = 1;
    	}
    	...
    	$this->assign(compact('sidebar_data','detail' ,'page_string','category','channel','prev','next' ,'hotnews','related','location' ,'goupon','relatedNews','show_fang_weixin'));
        $this->display("detail");
    }
    ...
    private function pagination_rolling($catid,$total,$tagid=0,$subpage=1,$page=1,$size=5,$tail,$type="index",$encode=true,$page_mark='#waterfall'){
		...
		else
		{
			...
			if($is_first){
				$html .= "<li ><a href='".$url_base."/1".$page_mark."' >1</a></li><li><a>...</a></li>";
			}
			...
			if($is_more){
				$html .= "<li><a>...</a></li><li ><a href='".$url_base."/".$pages.$page_mark."' >".$pages."</a></li>";
			}
		}
    ...
    function all_posts($subpage = 1,$page = 1){
    	...
    	$ugc_need_check = $this->config->item('ugc_need_check');
