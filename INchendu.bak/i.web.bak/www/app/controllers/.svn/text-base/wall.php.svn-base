...
class Wall extends MY_Controller {
    ...
    function post_list_place($name = '', $pids = '', $num = 20) {
        ...
        foreach($list as $row) {
            $data[] = array(
                ...
                'content' => $row['content'],
                'photo' => $row['photoName']?image_url($row['photoName'], 'user', 'thweb'):'',
    ...
    function list_post($name = '', $score = 0) {
        ...
        $list = $this->_redis->zRangeByScore($key, $score + 0.0001, $this->_get_score(), array('withscores' => true));
    ...
    private function _get_post($id) {
        ...
        if($post) {
            $data = array(
                ...
                'photo' => $post['photoName']?image_url($post['photoName'], 'user', 'thweb'):'',
