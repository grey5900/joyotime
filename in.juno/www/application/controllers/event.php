   		...
   		if($code = "wenming"){
			...
			if (! empty ( $apply_property ['keyword'] ))
			{
   			...
   			if($count){
				...
				//缓存！
				$list_keys = xcache_get($xcache_key);
				
				if(!$list_keys){
					$sql = "select a.id as id from `$table` a,`PostOwnTag` b where a.id=b.postId and $where and b.tagId in(".implode(',',$where_tagIds).") ";
					...
					xcache_set($xcache_key,$list_keys,3600*24);
					
				}
				
				$sql = "select * from `$table` where id in (".implode(',',$list_keys).")";
				$feeds = $this->db->query( $sql )->result_array();
				$list = $feeds ? $this->m_post->list_post(array(),'thweb',false,$feeds) : array();
